	...
	ns0#Envelope: {
		ns0#Body: {
			ns1#CalculateTransactionTotals: {
				ns1#ppMenuItems: {
					(payload.calculatetotals.menuItems.resposapiMenuItem map ((resposapiMenuItem , indexOfResposapiMenuItem) -> {
				...
				(ns1#pSubtotalDiscount: {
					ns1#DiscObjectNum: payload.calculatetotals.orderDiscount.discObjectNum as Number,
					ns1#DiscAmountOrPercent: payload.calculatetotals.orderDiscount.discAmountOrPercent as Number {
						format: "###.##"
					}
				}) if(payload.calculatetotals.orderDiscount.discObjectNum != null),
