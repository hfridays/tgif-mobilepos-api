...
fun getAuthCode() = using ( authCode = vars.createCheckVars.authCode ) if ( authCode != null and !(siEmpty(authCode) and getPaymentCardTypeName() == "AmazonPay") ) trim(authCode splitBy("-")[3]) else if ( getPaymentCardTypeName == "AmazonPay" ) "" else authCode
...
fun getCheckDetails(ppCheckPrintLines) = ((ppCheckPrintLines map (item, index) -> if ( item contains "Chk" ) {
...
fun getAmtPaidDetails(ppCheckPrintLines) = using ( string = ppCheckPrintLines ) using ( amtPaidDetails = (string filter ($ contains "Amt. Paid"))[0] splitBy " " filter ($ != "") ) amtPaidDetails[0] ++ " " ++ amtPaidDetails[1] ++ "(" ++ amtPaidDetails[2] ++ ")"
fun getTimestampIUndex() = ((ppCheckPrintLines map(item, index) -> if ( item contains "Chk" ) {
	index: index
} else {
}) filter ($ != {
}))[0].index + 1
fun getTime() = (((ppCheckPrintLines[getTimestampIUndex()] splitBy " ")[1]) splitBy /[A,P,M]/)[0]
fun getTimezone() = (((ppCheckPrintLines[getTimestampIUndex()] splitBy " ")[1]) splitBy /[\d]/)[4]
fun getCheckCreatedTime() = getTime() ++ ":" ++ now().milliseconds ++ getTimezone()
fun getPickupTime() = ""
fun mailHeaderFooter() = if ( getPaymentCardTypeName() == "PayInStore" ) "################\n################\n!! NOT PAID !!\n################\n################\n" 
else ""
---
if(ppCheckPrintLines != null) mailHeaderFooter() ++ "****************\n" ++ getOnlineOrderStore() ++ "\n" ++ getCheckCreatedTime() ++ "\n\n" ++ vars.fullfillmentOreder ++ "\n" ++ "****************" ++ 
getPickupTime() ++ "****************" ++ ppCheckPrintLines ++ "\n" ++ "Card Type: " ++  getPaymentCardTypeName() ++ "\n" ++ "Card # " ++ getLastFourDigits() ++ "\n" ++
